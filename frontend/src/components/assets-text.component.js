...
const Asset = props => (
  <tr>
    ...
    {props.user && props.user.isAdmin ? (
      <td>
        <Link to={"text/delete/" + props.asset._id}>
...
export default class assetText extends Component {
  ...
  onSubmitSearch = () => {
    if (this.state.type === "title") {
      axios
        .get(
          "http://localhost:4000/assets/searchTextTitleQuery/" + this.state.data
        )
        .then(res => {
      ...
      axios
        .get(
          "http://localhost:4000/assets/searchTextKeywordsQuery/" +
            this.state.data
        )
        .then(res => {
          this.setState({ assets: res.data });
      ...
      axios
        .get(
          "http://localhost:4000/assets/searchTextAuthorQuery/" +
            this.state.data
        )
        .then(res => {
          this.setState({ assets: res.data });
  ...
  onSubmitCancel = () => {
    axios
      .get("http://localhost:4000/assets/text")
      .then(response => {
